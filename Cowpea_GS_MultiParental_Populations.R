...
### get Phenotypes in rrBLUP format
  ...
  X <- GENO
##cross validation with putting back
...
#{
  for(r in 1:cycles)
  {
    ...
    tryCatch({
      ...
      print(paste("--------- Trait being analysed: ", phenames[l], " --cycle-- ", r, " ; GS Model: Fixed Effects RRBLUP"," !!!!!!!----------", sep = ""))
      library(rrBLUP)
      Yt <-Pheno_train
    ...
    #Ordinary RRBLUP
    print(paste("--------- Trait being analysed: ", phenames[l], " --cycle-- ", r, " ; GS Model: RRBLUP"," !!!!!!!----------", sep = ""))
    library(rrBLUP)
    Yt <-Pheno_train
    ...
    #RKHS 
    y.trn <- Pheno[,1] # for prediction accuracy
    ww <- Pheno[-train,1] # delete data for 1/5 of the population
    y.trn[-train] <- NA
    
    print(paste("--------- Trait being analysed: ", phenames[l], " --cycle-- ", r, " ; GS Model: RKHS"," !!!!!!!----------", sep = ""))
    ETA<-list(list(K=M,model='RKHS')) 
    fm.RK<-BGLR(y=y.trn,ETA=ETA,response_type="gaussian" ,nIter=Iter, burnIn=burn)
    Rk.accuracy[r,1] <- cor(fm.RK$yHat[-train], ww, use="complete")
    the.fitted.model.RK <- lm(ww ~ fm.RK$yHat[-train])
    ...
    these.observed.and.predicted.phenotypic.values.Rk <- data.frame(names(Y_valid), Y_valid, fm.RK$yHat[-train])
...
print(paste("--------- Summarizing Result for trait: ", phenames[l]," !!!!!!!----------", sep = ""))
    ...
    Rx.Trts <- rbind(Rx.Trts, Rx.accuracy)
    ...
    Sv.accuracy$Trt <- rep(phenames[l], nrow(Sv.accuracy))
    Sv.Trts <- rbind(Sv.Trts, Sv.accuracy)
    
    
}

#GS.Results <- rbind(Rx.Trts, Ro.Trts, Rk.Trts, Sv.Trts)


save.image("GS.Results.100cycles.AllTraits.rda")
#write.table(GS.Results, "GS.Results.100cycles.AllTraits.csv", sep=",", quote=F, row.names=F, col.names = T)


Rx.Trts$Model <- rep("FxRRBLUP", nrow(Rx.Trts))
Ro.Trts$Model <- rep("RRBLUP", nrow(Ro.Trts))
Rk.Trts$Model <- rep("RKHS", nrow(Rk.Trts))
Sv.Trts$Model <- rep("SVR", nrow(Sv.Trts))

GS.Results <- rbind(Rx.Trts, Ro.Trts,Rk.Trts, Sv.Trts)
colnames(GS.Results)[c(1:4)] <- c("P.Accuracy", "Intercept", "Slope", "C.Index")
GS.Results <- GS.Results[order(GS.Results$Trt, GS.Results$Model),]
write.table(GS.Results, "GS.Results.traits.Env.csv", sep=",", quote=F, row.names=F, col.names=T)
