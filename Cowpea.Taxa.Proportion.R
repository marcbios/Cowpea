     ...
     print(paste("-------------- Trait being analysed: ", phenames[l], "; Density: ",dens.vec[d]," !!!!!!!!!!!---------------", sep = ""))
        ...
        X <- GENO
          ...
          for(r in 1:cycles)
          {
            ...
            train=as.matrix(sample(1:nrow(Pheno),round(dens.vec[d]*nrow(Pheno))), replace=F)
            valid<-setdiff(1:nrow(Pheno),train)
            
            #set.seed(seed.vec2[r])
            valid2 <- sample(valid, round(0.2*nrow(Pheno)), replace = F)
            # taining set
            ...
            ww <- Pheno[valid2,1] # delete data for 1/5 of the population
            y.trn[valid2] <- NA
          ...
          Rx.accuracy$Density <- rep(dens.vec[d], nrow(Rx.accuracy))
          Rx.Density <- rbind(Rx.Density, Rx.accuracy)
...
save.image("GS.Results.Taxa.Proportion.FLT.GH.YLD.100cycles.rda")
#rm(list=ls())
load("GS.Results.Taxa.Proportion...100cycles.rda")
...
GS.Results <- rbind(Rx.Traits, Ro.Traits, Rk.Traits, Sv.Traits) # 
...
write.table(GS.Results, "GS.Results.New.Taxa.100cycles.FLT.GH.YLD..Proportion.csv", sep=",", quote=F, row.names=F, col.names=T)
